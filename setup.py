...
try:
    ...
    print("Module 'bpy' could not be imported. This probably means you are not \
            using Blender to run this script.")
    sys.exit(1)
...
subprocess.call([str(py_exec), "get-pip.py"])
...
for p in packages:
    subprocess.call([str(py_exec),"-m", "pip", "install", "--trusted-host", "pypi.python.org",
                    "--trusted-host", "files.pythonhosted.org", p])
