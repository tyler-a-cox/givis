...
class Visualization:
    """
    """
    def __init__(self, pos_file, var_file, output, mp = False, cores = 4, color_bins = 100,
                 color_map = plt.cm.inferno, x_res = 1920, y_res = 1080, cut = False):
        """
        """
    ...
    def set_render_settings(self, anaglyph = True, cyan = False):
        ...
        bpy.data.objects['Camera'].rotation_euler = (np.deg2rad(63.43), 0, np.deg2rad(63.43))
    ...
    def set_material_settings(self, obj_name, i = 0, halo_size = 0.04,
    ...
    def main_properties(self, i):
        ...
        for j, s in enumerate(sets):
            ...
            self.set_material_settings(obj_name, i = j, halo_size = 0.1,
                                       color = self.cmap[j][:3])
    ...
    def render(self):
        ...
        for i in np.arange(self.positions.shape[1]):
...
class Colorbar:
    """
    """
    def __init__(self, x_res = 1920, y_res = 1080, dpi = 300,
                 colormap = plt.cm.inferno, variable = 'temperature'):
        self.x_res = x_res
        self.y_res = y_res
        self.dpi = dpi
    ...
    def set_colorbar(self):
        ...
        s = plt.scatter(np.arange(20), np.arange(20), c = self.range[self.variable],
                        cmap = self.colormap, norm = colors.LogNorm())
        ...
        cbar.set_label(r'Temperature (K)', rotation = 270, labelpad = 22.,
                        fontsize = 18, color = 'white', fontweight = 'heavy')
        ax.remove()
...
parser.add_argument('--data_path',  '-p', type=is_path, required=True, help="Path to the data folder")
parser.add_argument('--variable', '-v', type=is_variable, required=True, help="Variable")
parser.add_argument('--size', '-s', default='640x320', help='Resolution size of the render')
parser.add_argument('--colorbar', '-cb', default=False, type=bool, help='Add a colorbar to the frame')
parser.add_argument('--bins', '-b', default = 100, type=int, help='Number of color bins')
parser.add_argument('--multiprocessing', '-mp', type=str, default=False, help="Use multiprocessing")
parser.add_argument('--cores', '-c', type=int, default=2, help="Number of cores used in multiprocessing")
...
if __name__ == '__main__':
    ...
    vis = Visualization(pos_file=pos_file, var_file=var_file, output=output, x_res=x_res,
                        y_res=y_res, mp=mp, cores=cores, color_bins=color_bins, cut=cut)
    vis.render()
